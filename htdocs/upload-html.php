...
if (isSet($_FILES['files']))
{
	...
	function codeToMessage($code)
	{
		...
		{
			case UPLOAD_ERR_OK:
				return 'There is no error, the file was '.
					'uploaded with success.';

			case UPLOAD_ERR_INI_SIZE:
				return 'The uploaded file exceeds the '.
					'‘upload_max_filesize’ directive in the ‘php.ini’.';
	...
	for ($i = 0; $i < $count; ++$i)
	{
		...
		{
			$errors[] = codeToMessage($error_code);
		}
		else
		{
			...
			echo '<pre>'; print_r($file_tmp); echo '</pre>';
			...
			if (!in_array($file_ext, $extensions))
			{
				...
				$errors[] = 'Please, choose a file with '.
		...
		if (empty($errors))
		{
			...
			echo '<h1>Success</h1>';
			$contents = HTML2RHeia::processFile($file_tmp);
...
;(function (document, window, index)
{
	...
	Array.prototype.forEach.call(forms, function(form)
	{
		...
		// Letting the server side to know we are going to make
		// an Ajax request:
		...
		if (isAdvancedUpload)
		{
			// Letting the CSS part to know drag&drop is supported
			// by the browser:
			form.classList.add('has-advanced-upload');

			['drag', 'dragstart', 'dragend', 'dragover',
				'dragenter', 'dragleave', 'drop'].forEach(function(event)
			{
		...
		{
			// Preventing the duplicate submissions if the current
			// one is in progress_:
			...
			if (isAdvancedUpload)
			{
				...
				ajax.open(form.getAttribute('method'),
					form.getAttribute('action'), true);
				...
				ajax.onload = function()
				{
					...
					if (ajax.status >= 200 && ajax.status < 400)
					{
						...
						form.classList.add(data.success == true ?
							'is-success' : 'is-error');
						if (!data.success)
